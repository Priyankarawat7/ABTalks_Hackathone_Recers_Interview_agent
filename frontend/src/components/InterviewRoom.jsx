...
// // Dynamic API URL with fallback to Vercel Production URL
// const API_BASE_URL = import.meta.env.VITE_BACKEND_URL || 'https://ab-talks-hackathone-recers-intervie.vercel.app';
...
//       setLoading(true);
//       try {
//         const res = await axios.post(`${API_BASE_URL}/api/interview`, {
//           sessionId,
//           candidate,
//         });
...
//         <div className="w-full max-w-6xl h-[88vh] flex flex-col md:flex-row bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
...
//                     AI Agent is evaluating response...
//                   </div>
//                 </div>
//               )}
...
export default function InterviewRoom({ candidate, sessionId, onEndInterview, onBack }) {
  ...
  useEffect(() => {
    const startInterview = async () => {
      ...
      try {
        ...
        console.error('Failed to start interview:', err);
        // Show detailed server error message on screen for debugging
        const errDetails = err.response?.data?.error || err.response?.data?.message || err.message || 'Unknown network error';
        setMessages([
          { 
            sender: 'agent', 
            text: `⚠️ [Backend Connection Error]: ${errDetails}\n\nPlease verify backend route "/api/interview" is deployed and CORS is enabled on Vercel.` 
          }
        ]);
      } finally {
        setLoading(false);
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      console.error('Error sending message:', err);
      const errDetails = err.response?.data?.error || err.message || 'Network issue';
      setMessages((prev) => [
        ...prev, 
        { sender: 'agent', text: `⚠️ [Error]: ${errDetails}. Please try sending again.` }
      ]);
    } finally {
      setLoading(false);
    }
          ...
          <div className="w-full md:w-80 bg-slate-950 p-6 border-b md:border-b-0 md:border-r border-slate-800 flex flex-col justify-between shrink-0">
            <div>
              ...
              <div className="mt-6">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {candidate.missions.slice(0, 5).map((m, i) => (
                    <div key={m.day || i} className="text-xs bg-slate-900 p-2.5 rounded-lg border border-slate-800 flex justify-between items-center">
                      <span className="text-slate-300 truncate max-w-[140px]">{m.title}</span>
                      <span className="text-[10px] text-emerald-400 font-mono font-bold">Day {m.day}</span>
                    </div>
                  ))}
